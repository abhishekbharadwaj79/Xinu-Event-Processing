...
struct  tentry  topicTab[TOPIC_TABLE_SIZE][ARRAY_SIZE];  /* Topic Table*/
struct  pqentry pendingQueue[ARRAY_SIZE];  /* Pending Queue */
...
static	void	sysinit()
{
	...
	/* Initialize topic table entries */

    int32   j;
    for (i = 0; i < TOPIC_TABLE_SIZE; i++) {
